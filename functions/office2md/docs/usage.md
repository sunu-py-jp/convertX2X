...
画像はブック・文書に埋め込まれたものだけを扱います。Excel・Wordの図形はグループの子要素まで個別に画像化し、重なり・接続・横並びを理由に合成しません。PowerPointは以下の文字を優先する出力を使います。

### PowerPointの図を検索できる文字で残す
...
接続はコネクターに保存された接続先IDと端点の矢印だけから生成します。近くの文字を分岐ラベルへ結び付けたり、座標から処理順や接続を推測したりしません。双方向と向きなしも区別します。保存情報がない・参照先が除外済み・IDが重複・端点記号の向きが不明な場合は、本文に「接続関係不明」、レポートに `DIAGRAM_CONNECTION_UNRESOLVED` を残します。複数の図形にコネクターが一つもない場合も、接続情報がないことを本文に示します。

`report.json` の `type: "diagram"` ブロックは、次の構造を持ちます。`shape-N` はスライド内のIDなので、全体では `section` と組み合わせて扱ってください。
...
画像の `nodes[].text` は保存済みの説明（`descr`、なければ `title`）です。画像内に焼き込まれた文字にはOCRを行わず、説明がなければ空文字です。図形の文字・説明の改行はJSON内で保持します。参考画像は図形を個別に分割せず、対応する表示要素をスライド全体の1枚へ描画します。回転・反転・グループ変形・重なり順を反映し、PNG/JPEGもこの参考画像に含めます。その他の画像形式は原本添付と警告にします。保存・継承された単色背景はテーマ参照を含めて反映し、背景指定がなければ白にします。画像・グラデーションなど未対応の背景は白へ置き換え、`UNSUPPORTED_SLIDE_BACKGROUND` を記録します。外部の背景画像は取得しません。

参考画像のaltは次の6項目のJSONです。`type` は「図」、`text` は空文字、`x / y` は0、幅・高さは入力スライドの寸法です。回転角はJSONに含めません。

```md
![{"type":"図","text":"","x":0,"y":0,"width":960,"height":540}](images/diagram-0001.png)
```

座標はスライド左上を原点、右をXの正方向、下をYの正方向とし、単位はpt（1/72インチ）で固定します。原点・単位のフィールドは出力しません。個別図形の寸法・文字は `nodes` にあり、参考画像のaltにはまとめ直しません。添付ファイルのリンクには個別の種類・文字・外接矩形のJSONを付けます。

Markdownから取得する場合はaltをそのまま `JSON.parse(alt)` に渡せます。文字中の角括弧・引用符・バックスラッシュなどは `\u005B`・`\u0022`・`\u005C` のようなJSONのUnicodeエスケープを使うため、Markdownのエスケープを別途解除する必要はありません。Excel・Wordのaltは従来どおり、日本語の形状種類・時計回りの回転角・文字・座標・サイズ・基準を含む説明文です。

RAGへの取り込みでは `[page n]` / H1を区切りにし、図中の項目と接続関係を同じチャンクへ残すと関係を保てます。長いため分割する場合も、接続だけを切り離さず参照先の文字を含め、未解決の関係を確定情報として扱わないでください。LLMによる説明やOCRは使いません。[業務フローのデモPPTX](../../../docs/APIDocs/office2md/examples/powerpoint-rag-flow/input.pptx) と [実変換Markdown](../../../docs/APIDocs/office2md/examples/powerpoint-rag-flow/output/document.md) で確認できます。
...
Excelは図形ごとの位置を使い、本文との上下順を保ち、表と重なる図形はその表の後へ置きます。Wordは本文XMLの順番です。PowerPointは本文・表の後に図中の項目と接続をまとめ、配置は参考画像で確認できます。通常のMarkdownビューアーやPlaygroundは座標による自由配置を行いません。
...
`report.json` は `specVersion: 2`、入力のファイル名・SHA-256、`sectionCount`・`sectionKind`、`warnings`、`information`、元の範囲と対応する `blocks`、出力画像のパス・種類・サイズ・SHA-256を持つ `assets` を含みます。PowerPointの `diagram` ブロックには `nodes / edges`、参考画像があれば `path / metadata` があり、添付ファイルにも `path / metadata` を付けます。画面に表示しきれない内容もファイルから確認できます。

[図形内文字の比較用Excel](../samples/shape-text.xlsx) と [指定内容の一覧](../samples/shape-text.md) で、文字色・3×3の上下左右配置・段落別配置・余白・折り返し・行間・縮小などを確認できます。

Word・PowerPointの図形も対応する基本形状を描画します。Word図形の文字は代表書式を用いる簡易描画で、図形内リンクの別出力には未対応です。段落ごとの混在書式・独自余白・文字だけの回転は近似として警告します。Wordの埋め込み画像は原本抽出で、切り抜き・回転を適用しません。PowerPointのPNG/JPEGは回転・反転・切り抜き・グループ内配置を参考画像へ反映し、個別の原本は抽出しません。それ以外の画像形式は原本添付のままで、回転などの加工は適用しません。グラフ、SmartArt、数式オブジェクトは未対応として警告し、外部データを取得して補完しません。参考画像にも対応要素だけを描画します。PowerPointの複雑な描画効果はPOIによる再現範囲に限られ、個々の見た目の差をすべて警告で検出するものではありません。旧形式の `.doc`・`.ppt`、PDF、マクロ有効形式・テンプレート、暗号化ファイルは対象外です。
