...
import java.nio.file.*;
import java.util.*;
public class OfficeDirectProducer {
 public static void main(String[] args) throws Exception {
  ...
  var queue=new QueueClientBuilder().connectionString(System.getenv("E2E_MAIN_STORAGE")).queueName("office2md-jobs").messageEncoding(QueueMessageEncoding.BASE64).buildClient();queue.createIfNotExists();queue.sendMessage(new ObjectMapper().writeValueAsString(request));
...
def write_evidence_index(directory, summary):
    ...
    lines = ["# 同一Officeファイルの実HTTP・Queue変換", "", "**" + ("PASS" if summary["passed"] else "FAIL") + "**", "",
             "入力資料を、ローカルFunctionsホストと別々のAzuriteアカウントで実処理しました。", "",
...
def run(args, work, tools):
    ...
    (work / "OfficeDirectProducer.java").write_text(PRODUCER_SOURCE, encoding="utf-8")
    ...
    try:
        ...
        def produce(mode, job_id):
            subprocess.run([tools["java"], "--class-path", classpath, str(work / "OfficeDirectProducer.java"),
...
def main():
    ...
    parser.add_argument("--input", type=Path, help="Convert this real .xlsx/.xls/.docx/.pptx through every route instead of generating the smoke fixture")
    ...
    parser.add_argument("--package-dir", type=Path, default=ROOT / "target" / "azure-functions" / "office2md-local")
    ...
    if args.input:
        if not args.input.is_file() or args.input.suffix.lower() not in (".xlsx", ".xls", ".docx", ".pptx"):
            parser.error("--input must be an existing .xlsx, .xls, .docx or .pptx file.")
    ...
    work = Path(tempfile.mkdtemp(prefix="office2md-async-e2e-", dir=args.temp_dir))
