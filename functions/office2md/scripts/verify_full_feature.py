...
folder. The optional OFFICE2MD_FUNCTIONS_KEY is sent in a header and is never recorded.
"""
...
def verify(output, files, request, limits, expected, checks):
    ...
    check("H1数とシート数の一致", len(headings) == report.get("sectionCount"), len(headings), report.get("sectionCount"))
...
def main():
    ...
    parser.add_argument("--key-env", default="OFFICE2MD_FUNCTIONS_KEY")
