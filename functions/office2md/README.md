...
非同期機能は `CONVERSION_STORAGE_CONNECTION_STRING` 設定時に有効になります。表・太字・リンクを保持します。PowerPointでは図中の文字と保存された接続関係も本文に出し、スライド画像を確認用に添えます。取消線の文字は除去し、数式の再計算や文書内の外部参照へのアクセスは行いません。
