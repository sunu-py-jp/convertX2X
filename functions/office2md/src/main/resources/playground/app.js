...
(() => {
  ...
  function inline(parent, source, assets, depth = 0, footnotes = new Map()) {
    ...
    for (let i = 0; i < source.length;) {
      ...
      if (reference && footnotes.has(reference[1])) {
        flush(); const sup = document.createElement('sup'), link = document.createElement('a');
        link.href = `#office-note-${reference[1]}`; link.textContent = footnotes.get(reference[1]).label;
        sup.append(link); parent.append(sup); i += reference[0].length; continue;
      }
      ...
      if (bold) {
        ...
        if (end >= 0) { flush(); const node = document.createElement('strong'); inline(node, source.slice(i + bold[0].length, end), assets, depth + 1, footnotes); parent.append(node); i = end + bold[1].length; continue; }
      }
      ...
      if (image || link) {
        ...
        if (end < source.length && close >= 0) {
          ...
          if (image) {
            ...
            if (href) { const node = document.createElement('a'); node.href = href; if (asset) node.download = destination.split('/').pop(); else { node.target = '_blank'; node.rel = 'noopener noreferrer'; } inline(node, label, assets, depth + 1, footnotes); parent.append(node); }
            else inline(parent, label, assets, depth + 1, footnotes);
          }
          i = close + 1; continue;
        }
      }
  ...
  function renderMarkdown(markdown, assets) {
    ...
    const definition = line => /^\[\^((footnote|endnote)-([0-9]+))\]: (.*)$/.exec(line);
    const footnotes = new Map();
    for (const line of lines) {
      const note = definition(line);
      if (note && footnotes.size < 10000 && !footnotes.has(note[1]))
        footnotes.set(note[1], { text: note[4], label: `${note[2] === 'footnote' ? '脚注' : '文末脚注'}${note[3]}` });
    }
    const content = (node, text) => inline(node, text, assets, 0, footnotes);
    const heading = line => /^(#{1,6}) (.+)$/.exec(line);
    const bullet = line => /^( {0,16})- (.*)$/.exec(line);
    let nodes = 0;
    for (let i = 0; i < lines.length && nodes < 10000;) {
      if (!lines[i].trim() || definition(lines[i])) { i++; continue; }
      const title = heading(lines[i]);
      if (title) { const h = document.createElement(`h${title[1].length}`); content(h, title[2]); i++; root.append(h); nodes++; continue; }
      if (bullet(lines[i])) {
        const lists = [];
        while (i < lines.length && nodes < 10000) {
          const item = bullet(lines[i]); if (!item) break;
          const level = Math.min(8, Math.floor(item[1].length / 2));
          while (lists.length && lists.at(-1).level > level) lists.pop();
          if (!lists.length || lists.at(-1).level < level) {
            const ul = document.createElement('ul');
            if (lists.length && lists.at(-1).last) lists.at(-1).last.append(ul); else root.append(ul);
            lists.push({ level, node: ul, last: null });
          }
          const li = document.createElement('li'); content(li, item[2]);
          lists.at(-1).node.append(li); lists.at(-1).last = li; i++; nodes++;
        }
        continue;
      }
      if (lines[i].startsWith('|') && lines[i].endsWith('|') && /^\|(?:\s*:?-+:?\s*\|)+$/.test(lines[i + 1] || '')) {
        ...
        const appendRow = (line, cellType) => { const row = document.createElement('tr'); for (const value of tableCells(line)) { if (++nodes > 10000) break; const cell = document.createElement(cellType); content(cell, value); row.append(cell); } table.append(row); };
      ...
      do { if (p.childNodes.length) p.append(document.createElement('br')); content(p, lines[i++].replace(/ {2}$/, '')); } while (i < lines.length && lines[i].trim() && !heading(lines[i]) && !bullet(lines[i]) && !definition(lines[i]) && !lines[i].startsWith('|'));
      root.append(p); nodes++;
    }
    if (footnotes.size) {
      const section = document.createElement('section'); section.className = 'footnotes'; section.setAttribute('aria-label', '脚注');
      for (const [id, note] of footnotes) {
        if (++nodes > 10000) break;
        const p = document.createElement('p'); p.id = `office-note-${id}`;
        const label = document.createElement('strong'); label.textContent = note.label + ': '; p.append(label);
        content(p, note.text); section.append(p);
      }
      root.append(section);
    }
  ...
  async function showResult(files, report, archive, job, run) {
    ...
    for (const warning of report.warnings.slice(0, 300)) {
      const li = document.createElement('li'); li.textContent = [warning.section, warning.range, warning.code, warning.message].filter(value => typeof value === 'string').join(' · '); $('warning-list').append(li);
    }
    ...
    const sectionLabel = ({ sheet: 'シート', slide: 'スライド', document: '文書' })[report.sectionKind] || 'セクション';
  ...
  async function perform(action, clear = true) {
    ...
    status('busy', clear ? 'Officeファイルを送信しています' : 'ZIPを取得しています', '通信中です。');
  ...
  function selectFile(file) {
    ...
    $('dropzone').classList.remove('has-file'); $('selected-name').textContent = 'Officeファイルをドロップ'; $('selected-meta').textContent = 'またはクリックしてファイルを選択';
    try {
