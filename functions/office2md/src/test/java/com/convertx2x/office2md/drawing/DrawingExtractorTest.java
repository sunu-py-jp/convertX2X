...
class DrawingExtractorTest {
    ...
    @Test void renderedDiagramByteLimitIsNotDowngradedToAWarning() throws Exception {
        ...
        var limit = new ConversionLimits(defaults.maxInputBytes(), defaults.maxSections(), defaults.maxReadItems(), defaults.maxTableCells(),
    ...
    @Test void documentKeepsTextBetweenIndividualGroupLeavesInPhysicalRowOrder() throws Exception {
        try (var book = new XSSFWorkbook()) {
            ...
            sheet.createRow(8).createCell(0).setCellValue("図形の間の本文");
            ...
            upper.setShapeType(ShapeTypes.ELLIPSE); upper.setText("上の図形");
            var bytes = new ByteArrayOutputStream(); book.write(bytes);
            try (var result = new ExcelMarkdownService(ConversionLimits.defaults()).convert(bytes.toByteArray(), "order.xlsx")) {
    ...
    private static BufferedImage image(ConversionWorkspace workspace, String path) throws Exception {
        return ImageIO.read(workspace.files().get(path).toFile());
    }
    ...
    private static ConversionLimits limits(ConversionLimits defaults, int shapes, int depth, long pixels, int images) {
        return new ConversionLimits(defaults.maxInputBytes(), defaults.maxSections(), defaults.maxReadItems(), defaults.maxTableCells(),
