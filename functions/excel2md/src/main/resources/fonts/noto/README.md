...
Noto Sans CJK JP（ゴシック）とNoto Serif CJK JP（明朝）のRegular・Boldを、Excelの図形内文字の代替フォントとして同梱します。元のフォントが利用できる場合は優先し、不足する日本語フォントを補います。Markdown本文の出力にはフォントを使いません。元の書体と字幅・改行・レイアウトが完全に一致するものではありません。
